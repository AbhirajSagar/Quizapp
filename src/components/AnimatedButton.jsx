...
export default function AnimatedButton({text = '', icon, onClick, hoverEffect = true,hideTextOnSmallScreens = true,disabled = false,className = '',iconAnim = 'none', layout = 'vertical'})
{
    ...
    return (
        <button onClick={() => onClick(1)} onMouseEnter={() => handleHover(true)} onTouchStart={() => handleHover(true)} onMouseLeave={() => handleHover(false)} onTouchEnd={() => handleHover(false)} className={`bg-accent-one ${layout === 'vertical' ? 'mt-2' : 'm-0 max-h-[95%] -translate-y-0.5'} shadow-blue-600 dark:shadow-dark-tertiary dark:bg-dark-secondary hover:bg-blue-900 transition-colors duration-150 hover:drop-shadow-md p-3 flex items-center text-white rounded text-sm font-semibold cursor-pointer shadow-[0px_5px_0px_0px] active:translate-y-[5px] active:shadow-[0px_0px_0px_0px] ${className}`} disabled={disabled}>
